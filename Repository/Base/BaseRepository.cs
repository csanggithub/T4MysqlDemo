    ...
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class
    {
        FlyingFishDB db
        {
            get
            {
                ...
                object obj = CallContext.GetData(typeof(FlyingFishDB).FullName);
                if (obj == null)
                {
                    //例化EF的上下文容器对象
                    obj = new FlyingFishDB();
                    //将EF的上下文容器对象存入线程缓存CallContext中
                    CallContext.SetData(typeof(FlyingFishDB).FullName, obj);
                }
                //将当前的EF上下文对象返回
                return obj as FlyingFishDB;
